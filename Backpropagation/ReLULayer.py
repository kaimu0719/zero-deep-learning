class Relu:
    def __init__(self):
        ...
        このレイヤーは、入力値が0より大きい場合はそのまま出力し、
        0以下の場合は0を出力する活性化関数を使用します。
        """

    def forward(self, x):
        """
        順伝播（forward propagation）
        入力された値（x）に対してReLUを適用し、その結果を返す。
        ReLUは、入力値が0より大きい場合はそのまま出力し、
        0以下の場合は0を出力する活性化関数です。
    ...
    def backward(self, dout):
        ...
        順伝播で作成したマスクを使用して、勾配（dout）を計算する。
